...
| `AG_CAVEATS` | `string[]` | Every unverified air-to-ground value (range bands, laser limit, gun, stations, Shkval FOV). |
...
- Laser limit conflict: S1 (trips at the temperature limit, cools as long as it was on, 20 min per flight) is
  implemented; the 1 min continuous figure from the research pass is not.
