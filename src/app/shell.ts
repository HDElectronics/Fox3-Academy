...
export function buildShell(root: HTMLElement, app: AppStore) {
  ...
  const nav = h('nav', { class: 'modnav', 'aria-label': 'Modules' },
    ROUTES.map(r => h('a', { href: '#/' + r.path, title: r.title, dataset: { path: r.path } }, r.label)));

  const bar = h('header', { class: 'topbar' },
    h('a', { class: 'brand', href: '#/hangar' }, h('span', { class: 'brand-mark' }, 'F3'), h('span', { class: 'brand-name' }, 'Fox Three School')),
    nav,
    h('div', { class: 'topbar-right' },
      h('label', { class: 'jet-label', for: 'jetSelect' }, 'Jet'), select, unitsBtn),
  );
  const outlet = h('main', { class: 'outlet', id: 'outlet' });
  root.replaceChildren(bar, outlet);
  ...
  const setActive = (route: RouteDef) => {
    for (const a of $$('a', nav)) a.setAttribute('aria-current', a.dataset.path === route.path ? 'page' : 'false');
