...
export function defaultSetup(ac: FighterId): SortieSetup {
...
export function parseSetup(ac: FighterId, raw: unknown): SortieSetup {
...
export function primaryMissile(ac: FighterId): MissileId | null {
...
const loadoutText = (ac: FighterId) => AIRCRAFT[ac].loadout.map(l => `${l.count}× ${MISSILES[l.missile].name}`).join(', ');

/** Launch zone head-on, both at their chosen altitudes and cruise Mach (the in-game DLZ numbers). */
export function headOnZone(missile: MissileId, shooter: FighterId, shooterAlt: number, target: FighterId, targetAlt: number): { rmax: number; rne: number } {
...
function threatLine(m: MissileId, shooter: FighterId, units: Units): string {
...
export function multiShot(ac: FighterId): string {
...
export function briefFacts(ac: FighterId, s: SortieSetup, units: Units): BriefFacts {
  ...
  const det = (obs: FighterId, obsAlt: number, tgt: FighterId, tgtAlt: number) => {
...
export function sortieWorld(ac: FighterId, s: SortieSetup): World {
